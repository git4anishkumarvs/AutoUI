...
def get_easyocr_reader(device="cuda"):
    global reader_cache
    use_gpu = device == "cuda" and torch.cuda.is_available()
    cache_key = "gpu" if use_gpu else "cpu"
    if cache_key not in reader_cache:
        reader_cache[cache_key] = easyocr.Reader(['en'], gpu=use_gpu)
    return reader_cache[cache_key]


def get_paddle_ocr(device="cuda"):
    global paddle_ocr_cache
    use_gpu = device == "cuda" and torch.cuda.is_available()
    cache_key = "gpu" if use_gpu else "cpu"
    if cache_key not in paddle_ocr_cache:
        paddle_device = "gpu" if use_gpu else "cpu"
        paddle_ocr_cache[cache_key] = PaddleOCR(
            lang='en',
    ...
    return paddle_ocr_cache[cache_key]


def extract_paddle_ocr_lines(result, text_threshold=0.5):
    coords = []
    texts = []
    ...
    def walk(node):
        ...
        if isinstance(node, dict):
            ...
            if rec_texts is not None and rec_scores is not None and rec_polys is not None:
                for text, score, poly in zip(rec_texts, rec_scores, rec_polys):
                    try:
                        score_value = float(score)
                    except Exception:
                        continue
                    if isinstance(text, str) and text.strip() and score_value >= text_threshold:
                        coords.append(poly)
                        texts.append(text)
                return
        ...
        if isinstance(node, (list, tuple)):
            if (
                ...
                try:
                    score_value = float(node[1][1])
                    text = node[1][0]
                    poly = node[0]
                except Exception:
                    score_value = None
                if score_value is not None and isinstance(text, str) and text.strip() and score_value >= text_threshold:
                    coords.append(poly)
                    texts.append(text)
                    return
...
def _ensure_generation_compat(model):
    targets = []
    for target in [getattr(model, "config", None), getattr(model, "generation_config", None)]:
        if target is not None:
            targets.append(target)
            nested_language = getattr(target, "language_config", None)
            if nested_language is not None:
                targets.append(nested_language)
            nested_text = getattr(target, "text_config", None)
            if nested_text is not None:
                targets.append(nested_text)

    for target in targets:
        if not hasattr(target, "forced_bos_token_id"):
            target.forced_bos_token_id = None
        if not hasattr(target, "forced_eos_token_id"):
            target.forced_eos_token_id = None
        if not hasattr(target, "decoder_start_token_id"):
            target.decoder_start_token_id = None
    return model
...
def get_caption_model_processor(model_name, model_name_or_path="Salesforce/blip2-opt-2.7b", device=None):
    ...
    model = _ensure_generation_compat(model)
...
def predict_yolo(model, image, box_threshold, imgsz, scale_img, iou_threshold=0.7, device="cuda"):
    """ Use huggingface model to replace the original model
    """
    # model = model['model']
    predict_kwargs = {
...
def check_ocr_box(
    ...
    if ocr_backend == "paddleocr":
        ...
        result = get_paddle_ocr(device=device).ocr(
            image_np,
            text_rec_score_thresh=text_threshold,
        )
        coord, text = extract_paddle_ocr_lines(result, text_threshold=text_threshold)
    else:
        result = get_easyocr_reader(device=device).readtext(image_np, **easyocr_args)
